...
namespace pkgrcpp {
    ...
    inline double approx2_pi(const int N) {
        typedef SEXP(*Ptr_approx2_pi)(SEXP);
        static Ptr_approx2_pi p_approx2_pi = NULL;
        if (p_approx2_pi == NULL) {
            validateSignature("double(*approx2_pi)(const int)");
            p_approx2_pi = (Ptr_approx2_pi)R_GetCCallable("pkgrcpp", "_pkgrcpp_approx2_pi");
        }
        RObject rcpp_result_gen;
        {
            RNGScope RCPP_rngScope_gen;
            rcpp_result_gen = p_approx2_pi(Shield<SEXP>(Rcpp::wrap(N)));
        }
        if (rcpp_result_gen.inherits("interrupted-error"))
            throw Rcpp::internal::InterruptedException();
        if (Rcpp::internal::isLongjumpSentinel(rcpp_result_gen))
